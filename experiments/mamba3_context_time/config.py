"""Frozen scientific settings, five explicit run IDs and private runtime paths."""
import json
...
ROOT = HERE.parents[1]
STUDY_ID = 'mamba3_context_time_001'
...
def plan():
    ...
    expected = [dict(mode=m, seed=2026, run_id=f'mamba3_context_{m}_seed2026_001') for m in MODES]
    if data['tasks'] != expected or data['study_id'] != STUDY_ID:
        raise ValueError('Frozen five-task plan mismatch')
    return data
...
def settings(task):
    result = load_config()
    result.update(seed=2026, time_mechanism_mode='separate', context_time_mode=task['mode'],
                  checkpoint_dir=str(paths(task)['checkpoint'].parent))
    return result


def scientific_settings(settings):
    result = {k: v for k, v in settings.items() if k not in ('checkpoint_dir', 'time_mechanism_mode', 'context_time_mode')}
