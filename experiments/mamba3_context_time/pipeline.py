"""Finite allocation: gates, five sequential fits, report; no scheduler calls/retries."""
import json
import os
import signal
import subprocess
...
from .config import GPU_EVIDENCE, HERE, ROOT, RUNTIME, STUDY_ID, SUMMARY, paths, plan
...
def main():
    ...
    try:
        for stage in ('preflight', 'gpu_checks'):
            ...
            run_child('experiments.mamba3_context_time.' + stage, [], RUNTIME / stage, deadline)
        ...
        for task in plan()['tasks']:
            ...
            atomic_json(output, identity)
            run_child('experiments.mamba3_context_time.run', ['--mode', task['mode']], paths(task)['runtime'], deadline)
        ...
        if active_task is not None:
            ...
            row = json.loads(p.read_text()) if p.exists() else dict(**active_task, test_evaluation_count=0)
            if row.get('status') != 'PASS':
        ...
        for task in plan()['tasks']:
            p = paths(task)['result']
            if not p.exists():
                atomic_json(p, dict(**task, study_id=STUDY_ID, status='NOT_RUN', test_evaluation_count=0,
        ...
        try:
            ...
            run_child('experiments.mamba3_context_time.report', [], RUNTIME / 'report', max(deadline, time.time()+180))
