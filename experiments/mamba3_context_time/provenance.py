...
def source_manifest():
    sources = sorted([*HERE.glob('*.py'), *(HERE / 'tests').glob('*.py'), HERE / 'study_plan.json',
                      ROOT / 'slurm/mamba3_context_time.sh'])
...
def verify():
    ...
    if sha(ROOT / reference['history_json']) != reference['history_sha256']:
        raise ValueError('Historical separate history changed')
...
def backbone_hash(model):
    return tensor_hash({k: v for k, v in model.state_dict().items() if not k.startswith('mechanisms.')})
