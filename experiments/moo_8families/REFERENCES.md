...
- Validation Pareto hypervolume uses one frozen evaluation metric-space reference from `config.yaml`: `[1.0, 2.0, 2.0, 2.0, 2.0]` for `[1-NDCG@10, click_BCE, long_view_BCE, like_BCE, profile_BCE]`. The ranking coordinate keeps the analytic `1-NDCG@10` upper bound, while BCE coordinates use a conservative fixed pre-MOO bound because BCE is unbounded above. Existing historical/control validation records had maximum BCE `0.6414133221375151` and no BCE above `2.0`; the reference is never recomputed per method or from new MOO outputs.
- Primary reporting uses `ranking_operating_point`, not the oracle over preference grid. EPO/PHN-adapter/COSMOS-style/PaLoRA use predefined `rank_heavy`; HV-Gradient / GradHV-style is explicitly marked as preference-free validation-oracle selection.
