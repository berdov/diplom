...
from .provenance import acquire, atomic_json, backbone_hash, historical_backbone_hash, now, require_submission, rng_hash, sha, tensor_hash
...
def train(task, record, task_paths):
    ...
    data = settings(task)
    config = Config(model=InputMamba3Rec, config_dict=data)
    ...
    model = InputMamba3Rec(config, train_ds).to(config['device'])
    record.update(protocol=protocol, train_time_stats_sha256=STATS_SHA, manifest_sha256=MANIFEST_SHA,
                  ...
                  adapter_initialization_sha256=tensor_hash(model.input_adapter.state_dict()),
                  attention_common_initialization_sha256=(tensor_hash(model.input_adapter.common_state()) if task['mode'].startswith('attention_') else None),
    ...
    if (protocol != reference['protocol'] or history_stats != reference['verified_history_stats']
            or historical_backbone_hash(model) != reference['initial_backbone_sha256']):
        raise ValueError('Historical dataset/backbone initialization drift')
    record['historical_backbone_sha256'] = historical_backbone_hash(model)
    ignored = {'model','context_time_mode','input_time_mode','checkpoint_dir'}
    current = {k:str(v) for k,v in config.final_config_dict.items() if k not in ignored}
    old = {k:str(v) for k,v in reference['effective_config'].items() if k not in ignored}
    if current != old:
        raise ValueError('Historical effective config drift: '+str(sorted(k for k in set(current)|set(old) if current.get(k)!=old.get(k))))
    replay = None
    ...
    if task['mode'] == 'attention_time':
        content = json.loads(paths(plan()['tasks'][2])['result'].read_text())
        if content['status'] != 'PASS' or record['attention_common_initialization_sha256'] != content['attention_common_initialization_sha256']:
            raise ValueError('Attention common initial weights differ')
    cls = trainer_class(Trainer, valid_data, record, task_paths,
                        reference['first_train_batch_sha256'] if replay is None else replay['first_train_batch_sha256'])
    ...
    record.update(stage='TRAIN_VALID', rng_before_fit_sha256=rng_hash(),
                  ...
                  epoch_indexing='zero-based', historical_rng_evidence='Compared with published context separate replay hashes')
    try:
        if reference['rng_before_fit_sha256'] != record['rng_before_fit_sha256']:
            raise ValueError('Historical pre-fit RNG drift')
        ...
        record.update(runtime_seconds=time.perf_counter()-started,
...
def main():
    ...
    from .gpu_checks import require_gpu_evidence
    require_gpu_evidence(evidence,manifest)
    ...
    record = dict(**task, study_id=STUDY_ID, status='RUNNING', stage='PREFLIGHT',
                  ...
                  test_status='NOT_RUN', test_evaluation_count=0, paths={k: str(v) for k,v in task_paths.items()})
