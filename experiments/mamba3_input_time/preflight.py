...
def effective_settings_check(config, task):
    ...
    return 'PASS (except model name and input_time_mode)'
...
def full_counts():
    ...
    from .adapter import COUNTS, ADAPTER_COUNTS
    rows = []
    for task in plan()['tasks']:
        cfg = Config(model=InputMamba3Rec, config_dict=settings(task))
        effective_settings_check(cfg, task)
        init_seed(2026, True)
        model = InputMamba3Rec(cfg, Catalog())
        count = sum(p.numel() for p in model.parameters())
        added = sum(p.numel() for p in model.input_adapter.parameters())
        if (count, added) != (COUNTS[task['mode']], ADAPTER_COUNTS[task['mode']]):
            raise ValueError('Actual count mismatch')
        rows.append(dict(mode=task['mode'], total=count, adapter=added,
            backbone=backbone_hash(model), rng=rng_hash(),
            attention_common=tensor_hash(model.input_adapter.common_state()) if task['mode'].startswith('attention_') else None))
    if len({r['backbone'] for r in rows}) != 1 or len({r['rng'] for r in rows}) != 1 or rows[2]['attention_common'] != rows[3]['attention_common']:
