...
def validate(row, task, source_hash):
    expected = {**task, 'study_id': STUDY_ID, 'core_hash': CORE, 'source_hash': source_hash,
                'selection_split': 'VALID', 'evaluation_mode': 'full-ranking', 'test_evaluation_count': 0, 'test_status': 'NOT_RUN'}
    ...
    for key in ('runtime_seconds','peak_cuda_allocated_bytes','peak_cuda_reserved_bytes'):
        if not math.isfinite(row[key]) or row[key] <= 0:
            raise ValueError('Missing/nonfinite runtime or memory evidence')
...
def summarize(records, source_hash):
    ...
    for task in plan()['tasks']:
        ...
        if row is not None and row.get('status') == 'PASS':
            ...
            view.update(parameters=row['parameter_count'], ndcg10=row['best_valid_score'],
                ...
                runtime_seconds=row['runtime_seconds'], peak_cuda_allocated_bytes=row['peak_cuda_allocated_bytes'],
    ...
    if complete:
        ...
        comparisons = {f'attention_time_minus_{k}': v['attention_time']-v[k] for k in ('attention_content','time_add')}
        comparisons.update({f'{k}_minus_separate_replay': v[k]-v['separate_replay'] for k in ('time_add','attention_content','attention_time')})
        interpretation = []
        if v['attention_time'] > v['attention_content']:
            interpretation.append('В этом единственном запуске временной attention выше content control; устойчивость и значимость не установлены.')
        else:
            interpretation.append('Дополнительная польза времени в attention по NDCG@10 не подтверждена.')
        if v['time_add'] >= v['attention_time']:
            interpretation.append('Временной attention не обосновал усложнение относительно time_add по NDCG@10.')
        if all(v[k] < v['separate_replay'] for k in ('time_add','attention_content','attention_time')):
            interpretation.append('Все новые режимы ниже separate: автоматически не усложнять модель.')
        interpretation.append('Выигрыш только над replay не доказывает пользу временного attention. Новая основная модель автоматически не выбирается.')
...
def render(result, output):
    ...
    element('title', 'VALID NDCG@10: input-time pilot, seed2026', id='title')
    ...
    label('Input-time pilot: seed2026', 540, 30, text_anchor='middle')
...
def main():
    ...
    lines = ['# Причинный входной attention/time адаптер Mamba3', '', result['limitations'], '',
    ...
    for r in result['rows']:
        memory = '/'.join(str(round(r[k]/2**20)) for k in ('peak_cuda_allocated_bytes','peak_cuda_reserved_bytes')) if r['status']=='PASS' else '-'
        runtime = round(r['runtime_seconds']) if r['status']=='PASS' else '-'
        lines.append(f"| {r['mode']} | {r.get('parameters','-')} | {fmt(r.get('ndcg10'))} | {fmt(r.get('hr10'))} | {fmt(r.get('delta_replay'))} | {r.get('best_epoch','-')} | {r.get('actual_epochs','-')} | {runtime} | {memory} | {r['status']} |")
    ...
    lines += [f"- {key}: {value:+.4f}" for key,value in result['comparisons'].items()]
