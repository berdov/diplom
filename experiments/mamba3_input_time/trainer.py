...
import os
import time
from .diagnostics import InputDiagnostics
...
def trainer_class(base, valid_loader, record, paths, expected_first_batch=None):
    class ValidationTrainer(base):
        def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
            if self.model.diagnostic_collector is not None:
                raise ValueError('Diagnostics must be disabled during TRAIN')
        ...
        def _valid_epoch(self, valid_data, show_progress=False):
            collector = InputDiagnostics(record['mode'])
