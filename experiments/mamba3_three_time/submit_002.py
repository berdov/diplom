"""Attempt002 reservation, exactly one sbatch; never retries or monitors."""
...
def main():
    ...
    record = dict(attempt_id="002",status="RESERVED",execution_commit=args.commit,
        parent_execution_commit=attempt.PARENT,source_hash=manifest["source_hash"],core_hash=CORE,
        plan_sha256=sha(attempt.PLAN),branch="exp/mamba3-three-time",jobs_submitted=None,
    ...
    try:
        response = subprocess.run(["sbatch","--parsable","slurm/mamba3_three_time_correctness_002.sh"],
