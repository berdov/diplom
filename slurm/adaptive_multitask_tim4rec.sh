#!/usr/bin/env bash
#SBATCH --job-name=adaptive-mtl-smoke
...
RUN_ID="${ADAPTIVE_MTL_RUN_ID:-adaptive_smoke_001}"
...
BATCHES="${ADAPTIVE_MTL_SMOKE_BATCHES:-3}"
...
"${PYTHON}" experiments/adaptive_multitask_tim4rec/smoke_test.py \
  --config "${CONFIG}" \
  --run-id "${RUN_ID}" \
