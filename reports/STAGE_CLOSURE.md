...
## Обновление 10 сентября, 21:15 МСК

`4317564_1` / `target_confirm_0011_seed2027_001` завершился FAILED (1:0) до обучения: RecBole одновременно с другим процессом создавал общую папку `./log`; `os.makedirs` выбросил `FileExistsError`. Это техническая гонка и **no scientific result**, а не результат качества like + profile_enter. Исходный failure JSON сохранён. TEST=0; перезапуск не выполнялся.

Задачи 0 и 2 уже обучаются, задачи 3–5 ожидают ресурсов. До отдельного решения о восстановлении отсутствующего seed полнота 9/9 недостижима; итоговая сводка не должна выдавать неполный набор за завершённое подтверждение. Remote checkout и scientific settings не изменялись.
