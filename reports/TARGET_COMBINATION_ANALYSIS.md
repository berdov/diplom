...
Все 16 scientific runs внесены в [canonical CSV](../experiments/results.csv) с `record_type=target_combination_validation_screening`; [canonical summary](RESULTS.md#target-combination-screening). Отдельный smoke не включён в 16/16 и canonical метрики. [Аудит переноса](CANONICAL_RESULTS_AUDIT.md) · [Исторический Stage 3](STAGE3_AUXILIARY_ANALYSIS.md).
...
Historical all-four diagnostic `stage3_all_current_aux_diagnostic_001` с NDCG@10 **0.0597** сохранён отдельно: он использовал `tuned_task_weights`, тогда как здесь all-four **0.0589** получен с `uniform_normalized_aux`. Это разные loss-конфигурации; их различие не является регрессией одной и той же настройки. Stage 3 primary-only **0.0586** также сохранён, но не используется как baseline этого screening.

Screening завершён, multi-seed подтверждение ещё не выполнено. Следующий контроль для отбора subset — primary-only, like + profile_enter и click + like + profile_enter при общем VALID-протоколе; ни одна комбинация пока не объявляется окончательной конфигурацией по статистически подтверждённому преимуществу.
